...
(defn pokemon-input
  ...
  (let [pokemon-name (re-frame/subscribe [::subs/pokemon-name])
        search #(re-frame/dispatch [::re-graph/query
                                    q/pokemon
                                    {:name @pokemon-name}
                                    [::events/fetch-pokemon]])]
    [:div
     [:label   "Pokemon: "
      [:input {:value @pokemon-name
               :on-key-down #(case (.-which %)
                               13 (search)
                               nil)
               :on-change #(re-frame/dispatch [::events/update-pokemon-name (utils/get-target-value %)])}]]
  ...
  (let [name (re-frame/subscribe [::subs/name])]
    [:div
     ...
     [:div {:style {:display "flex"
                    ...
                    :flex-wrap "wrap"}}
      [:div
       [:h3 "List of pokémons available:"]
       [pokemon-list]]
      [:div
       [:h2 "Get infos about a pokémon"]
       [pokemon-input]
       [pokemon-info]]]]))
